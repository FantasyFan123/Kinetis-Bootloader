...
__attribute__((section(".init")))const Byte str_app_ok[8]	= "APP_OK";
__attribute__((section(".init")))const Byte station_number	= 0x01;
...
__attribute__((section(".init")))const IdentType MCU_Identification = {BL_M0, "MKL26Z4","1.0" ,0x400, 0x40, 0x00020000, 0x001000, 0x0003FC, 0x0003FF};
...
int main(void)
{	
    volatile LWord loop_counter = 0;  // counter of loop without received frame
	Byte enableBootMode = 0; 			//  0: enter APP, 1: enter BOOT mode, 2: enter verify mode

	if (( RCM_SRS0 & RCM_SRS0_POR_MASK)) // if reset is power on reset, set AppIDC as 0
		AppIDC = 0x00000000;
	...
	{  
		...
		volatile Byte frame_length 	= 0;    // SCI received frame length, initialized as 0xFF
		volatile Byte data_length = 0;
		volatile Byte buff_index = 0;   	   // receive buffer index for sci_buffer[]
		...
		{
			sci_buffer[1] = UART_GetChar();     // sci_buffer[1] is node address, for using bootloader with multi nodes.
			UART_GetChar();   					// sci_buffer[2] is reserved for future use
		...
		{ 
			...
			Byte burn_data_buffer[WR_BLOCK_BYTE]; //extract app file burning code from received frame, store them to burn_data_buffer[]
			...
			switch(sci_buffer[4])
			{
				...
				case'W':	// receive 'W' command, extract app  burning code,  program flash. then send confirm frame to UART
							...
							for(j=0,i=9;j<burn_data_length;j++,i++) // extract the prepared writing data from sci_buff[] to burn_data_buffer[]
							{
								 burn_data_buffer[j] = sci_buffer[i];   
							}
							if(!(FLASH_ProgramSectionByLongs (address.complete, (LWord*) burn_data_buffer, burn_data_length/4))) 
							{            
								 sci_buffer[3] = 00;
								 Boot_Send(sci_buffer);
								 break;
		...
		loop_counter ++;
		if(loop_counter >= 10 && enableBootMode == 2)                         // 10 * timeout of UART_GetChar();
		{
			Boot_Cpu_SystemReset( );
		}
