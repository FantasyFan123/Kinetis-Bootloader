...
{
	    Byte targ_core;
		Byte targ_name[20];	                                 // target name
        Word erblk;			                                 // erase block size
        Word wrblk;			                                 // write block size
        AddressType addr_limit;								 // MCU maximal FLASH address
        AddressType boot_addr_start;						 // bootloader start address
        AddressType boot_addr_end;							 // bootloader end addrss
...
__attribute__((section(".init")))const IdentType MCU_Identification = {BL_M0, "MKL26Z128", 0x400, 0x40, 0x00020000, 0x000000, 0x000FFF, 0x0003FC, 0x0003FF};
...
{	
	...
	{  
		...
		FLASH_EraseSector((LWord)APPOK_START_ADDRESS);
		...
		{ 
			...
			switch(sci_buffer[4])
			{
				...
				case 'I':	// receive 'I' command, send  version information to UART    
		                    ...
		                    j = i;
		                    while((sci_buffer[i] = Boot_version[i-j]) !='\0')	//assign bootloader version info to sci_buffer[]
		                    {
		                        i++;
		                    }
		                    j = 0;
		                    sci_buffer[i++] = WR_BLOCK_BYTE/256;
		                    sci_buffer[i++] = WR_BLOCK_BYTE%256;

		                    sci_buffer[i++] = ER_BLOCK_BYTE/256;
		                    sci_buffer[i++] = ER_BLOCK_BYTE%256;

		                    sci_buffer[i++] = FlashSize/65536;
		                    sci_buffer[i++] = FlashSize%65536/256;
		                    sci_buffer[i++] = FlashSize%65536%256;

		                    sci_buffer[i++] = Non_volatile_address_start/65536;
		                    sci_buffer[i++] = Non_volatile_address_start%65536/256;
		                    sci_buffer[i++] = Non_volatile_address_start%65536%256;

		                    sci_buffer[i++] = Non_volatile_address_end/65536;
		                    sci_buffer[i++] = Non_volatile_address_end%65536/256;
		                    sci_buffer[i++] = Non_volatile_address_end%65536%256;

		                    sci_buffer[i++] = RELOCATED_VECTORS/65536;
		                    sci_buffer[i++] = RELOCATED_VECTORS%65536/256;
		                    sci_buffer[i++] = RELOCATED_VECTORS%65536%256;
