    ...
    { 
        ...
        public int enter_boot(int mode)
        {
            ...
            if (resp_buffer[0] == '$' && resp_buffer[1] == StationAddr && resp_buffer[4 + rcv_datalength] == 0xAA && resp_buffer[4 + rcv_datalength +1] == 0x55)
            {
                ...
                MyVar.ident.wrblk = (uint)resp_buffer[i++] * 256 + (uint)resp_buffer[i++];

                MyVar.ident.erblk = (uint)resp_buffer[i++] * 256 + (uint)resp_buffer[i++];

                MyVar.ident.addr_limit = (uint)resp_buffer[i++] * 65536 + (uint)resp_buffer[i++] * 256 + (uint)resp_buffer[i++];

                MyVar.ident.dontcare_addrl = (uint)resp_buffer[i++] * 65536 + (uint)resp_buffer[i++] * 256 + (uint)resp_buffer[i++];

                MyVar.ident.dontcare_addrh = (uint)resp_buffer[i++] * 65536 + (uint)resp_buffer[i++] * 256 + (uint)resp_buffer[i++];

                MyVar.ident.relocated_vectors = (uint)resp_buffer[i++] * 65536 + (uint)resp_buffer[i++] * 256 + (uint)resp_buffer[i++];
